...
import approxtorch as at
import math
from .quantizer import LSQQuantizer
...
def approx_conv2d(feature, weight, lut, scale_feature, scale_weight, kernel_size, padding, stride, dilation, output_shape):
    ...
    output = output * scale.view(1, -1, 1)
...
class Conv2dBN_qat_int8(torch.nn.Module):
    def __init__(self, 
        ...
        # LSQ scales
        self.scale_feature = nn.Parameter(torch.empty([]))
        self.scale_weight = nn.Parameter(torch.empty(out_channels))
        ...
        # Output quantizer
        self.output_quantizer = LSQQuantizer(bit_width=8, symmetric=True)
        
        self._init_parameters()
        
    # ========== BN 控制方法 ==========
    def freeze_bn_stats(self):
        """冻结 BN，不再更新 running_mean/var"""
        self.freeze_bn.fill_(True)
    
    def unfreeze_bn_stats(self):
        """解冻 BN，恢复更新 running_mean/var"""
        self.freeze_bn.fill_(False)
    
    @property
    def is_bn_frozen(self):
        return self.freeze_bn.item()
    
    @property
    def update_bn_stats(self):
        """是否需要更新 BN 统计量"""
        return self.training and not self.freeze_bn.item()  
    ...
    def _init_parameters(self):
        ...
        nn.init.constant_(self.scale_feature, 0.1)
        nn.init.constant_(self.scale_weight, 0.1)
        
    def __repr__(self):
        return (f"Conv2dBN_qat_int8(in={self.in_channels}, out={self.out_channels}, "
                f"k={self.kernel_size}, s={self.stride}, p={self.padding}, d={self.dilation})")
    ...
    def forward(self, x):
        ...
        if self.training and not self.freeze_bn:
            # 计算 batch 统计量
            with torch.no_grad():
                conv_out = approx_conv2d(
                    x, self.weight, self.lut,
                    self.scale_feature, self.scale_weight,
                    kernel_size, self.padding, self.stride, self.dilation, output_shape
                )
            ...
            weight_factor = self.gamma / r_std
            weight_fold = self.weight * weight_factor.view(-1, 1, 1, 1)
            scale_weight_fold = self.scale_weight * weight_factor
            
            # 近似卷积
            output = approx_conv2d_int8_lsq_ste(
                x, weight_fold, self.lut,
                self.scale_feature, scale_weight_fold,
                kernel_size, self.padding, self.stride, self.dilation, output_shape
            )
            
            # BN correction
            output_factor = (r_std / batch_std).view(1, -1, 1, 1)
            bias_fold = (self.beta - self.gamma * batch_mean / batch_std).view(1, -1, 1, 1)
            output = output * output_factor + bias_fold
        else:
            # Inference: 用 running stats
            r_std = torch.sqrt(self.running_var + self.eps)
            weight_factor = self.gamma / r_std
            weight_fold = self.weight * weight_factor.view(-1, 1, 1, 1)
            scale_weight_fold = self.scale_weight * weight_factor
            
            output = approx_conv2d(
                x, weight_fold, self.lut,
                self.scale_feature, scale_weight_fold,
                kernel_size, self.padding, self.stride, self.dilation, output_shape
            )
            bias_fold = (self.beta - self.gamma * self.running_mean / r_std).view(1, -1, 1, 1)
            output = output + bias_fold
