    ...
    def forward(
        ...
        match qmethod:
            case (_, 'tensor', 'tensor'):
                ...
                term1 = zero_feature * qweight.sum(dim=(1,2,3)).view(1, -1, 1, 1)
                output = output - term1
...
def conv2d_uint8_ste(feature, weight, lut, qmethod: tuple[str, str, str]=('dynamic', 'tensor', 'tensor'), scale_feature: torch.Tensor | None = None, zero_feature: torch.Tensor | None = None, scale_weight: torch.Tensor | None = None, zero_weight: torch.Tensor | None = None, bias = None, stride: int | tuple[int, int] = 1, padding: int | tuple[int, int] = 0, dilation: int | tuple[int, int] = 1, groups: int = 1):
    return _conv2d_uint8_ste.apply(feature, weight, lut, qmethod, scale_feature, zero_feature, scale_weight, zero_weight, bias, stride, padding, dilation, groups)

class Conv2d_uint8_STE(torch.nn.Module):
    def __init__(self, 
            in_channels: int,
            out_channels: int,
    ...
    def forward(self, x):
        ...
        return conv2d_uint8_ste(x, self.weight, 
                                self.lut, 
                                self.qmethod, 
                                self.scale_feature, 
                                self.zero_feature, 
