...
//
// Also provides the ASYMMETRIC (affine) per-tensor variant for unsigned
// ranges (uint8: qmin=0, qmax=255), fakequant_per_tensor_asymmetric_claude:
//   forward:  v = x/s + z; mask = (v >= qmin) & (v <= qmax);
//             q = clamp(round(v), qmin, qmax)      (integer-domain output,
//                                                    dequant = (q - z) * s)
//   backward: z is additive so dq/dx = mask / s — identical to the
//             symmetric STE backward; the registered asymmetric backward op
//             simply reuses the same kernel.
// zero_point is a 1-element float32 CUDA tensor (integer-valued by
// convention, so that real 0 maps exactly onto the grid); like scale it is
// not learned through autograd.
...
// Asymmetric: v = x/s + z (round after adding z, matching
// torch.fake_quantize_per_tensor_affine). Mask on the pre-round v, same
// convention as the symmetric path.
__device__ __forceinline__ float fq_one_asym(float xv, float s, float z,
                                             float qmin, float qmax, bool& m) {
    float v = xv / s + z;
    m = (v >= qmin) && (v <= qmax);
    float r = nearbyintf(v);             // round half to even == torch.round
...
// Asymmetric forward: same structure as fakequant_pt_fwd_kernel, plus the
// broadcast zero_point. Backward needs no dedicated kernel — the asymmetric
// STE gradient is gx = (mask ? go : 0) / s, i.e. fakequant_pt_bwd_kernel.
template <int VEC>
__global__ void fakequant_pt_asym_fwd_kernel(
    const float* __restrict__ x,
    const float* __restrict__ scale,       // 1 element
    const float* __restrict__ zero_point,  // 1 element
    float* __restrict__ q,
    bool* __restrict__ mask,
    long long numel, float qmin, float qmax
) {
    const float s = fmaxf(scale[0], 1e-12f);
    const float z = zero_point[0];
    const long long stride = (long long)gridDim.x * blockDim.x;
    const long long base = (long long)blockIdx.x * blockDim.x + threadIdx.x;
    ...
    if constexpr (VEC == 4) {
        ...
        for (long long i = base; i < n4; i += stride) {
            float4 v = x4[i];
            float4 r; bool m0, m1, m2, m3;
            r.x = fq_one_asym(v.x, s, z, qmin, qmax, m0);
            r.y = fq_one_asym(v.y, s, z, qmin, qmax, m1);
            r.z = fq_one_asym(v.z, s, z, qmin, qmax, m2);
            r.w = fq_one_asym(v.w, s, z, qmin, qmax, m3);
            q4[i] = r;
            m4[i] = make_uchar4(m0, m1, m2, m3);
        }
    }
    for (long long i = (n4 << 2) + base; i < numel; i += stride) {
        bool m;
        q[i] = fq_one_asym(x[i], s, z, qmin, qmax, m);
        mask[i] = m;
    }
}
...
// Asymmetric (affine) fake-quant to an unsigned range, e.g. uint8 with
// qmin=0, qmax=255. Returns (q, mask); q holds integer values in float32
// (dequant on the caller side is (q - z) * s). zero_point: 1-element fp32
// CUDA tensor, integer-valued by convention (not enforced here).
std::tuple<torch::Tensor, torch::Tensor> fakequant_per_tensor_asymmetric_claude(
    torch::Tensor x, torch::Tensor scale, torch::Tensor zero_point,
    int64_t qmin, int64_t qmax
) {
    TORCH_CHECK(x.is_cuda() && scale.is_cuda() && zero_point.is_cuda(),
                "x, scale and zero_point must be CUDA");
    TORCH_CHECK(x.scalar_type() == torch::kFloat32, "x must be float32");
    TORCH_CHECK(scale.scalar_type() == torch::kFloat32, "scale must be float32");
    TORCH_CHECK(zero_point.scalar_type() == torch::kFloat32, "zero_point must be float32");
    TORCH_CHECK(scale.numel() == 1, "scale must be a scalar tensor");
    TORCH_CHECK(zero_point.numel() == 1, "zero_point must be a scalar tensor");

    const at::cuda::OptionalCUDAGuard device_guard(device_of(x));
    auto xc = x.contiguous();
    auto sc = scale.contiguous();
    auto zc = zero_point.contiguous();
    auto q = at::empty_like(xc);
    auto mask = at::empty(xc.sizes(), xc.options().dtype(torch::kBool));
    ...
    if (n > 0) {
        ...
        const bool vec = fq_aligned(xc.data_ptr<float>(), 16) &&
                         ...
                         fq_aligned(mask.data_ptr<bool>(), 4);
        if (vec) {
            fakequant_pt_asym_fwd_kernel<4><<<fq_grid(n / 4 + 1), FQ_THREADS, 0, stream>>>(
                xc.data_ptr<float>(), sc.data_ptr<float>(), zc.data_ptr<float>(),
                q.data_ptr<float>(), mask.data_ptr<bool>(),
                n, (float)qmin, (float)qmax);
        } else {
            fakequant_pt_asym_fwd_kernel<1><<<fq_grid(n), FQ_THREADS, 0, stream>>>(
                xc.data_ptr<float>(), sc.data_ptr<float>(), zc.data_ptr<float>(),
                q.data_ptr<float>(), mask.data_ptr<bool>(),
                n, (float)qmin, (float)qmax);
        }
...
torch::Tensor fakequant_per_tensor_asymmetric_backward_claude(
    torch::Tensor grad_output, torch::Tensor mask, torch::Tensor scale
) {
    // zero_point is additive: dq/dx = mask / s, exactly the symmetric STE
    // backward — reuse it (same clamped scale, same kernels).
    return fakequant_per_tensor_backward_claude(grad_output, mask, scale);
}
...
TORCH_LIBRARY_FRAGMENT(approxtorch, m) {
    ...
    m.def("fakequant_per_tensor_asymmetric_claude(Tensor x, Tensor scale, Tensor zero_point, int qmin, int qmax) -> (Tensor, Tensor)");
    m.def("fakequant_per_tensor_asymmetric_backward_claude(Tensor grad_output, Tensor mask, Tensor scale) -> Tensor");
}
...
TORCH_LIBRARY_IMPL(approxtorch, CUDA, m) {
    ...
    m.impl("fakequant_per_tensor_asymmetric_claude", &fakequant_per_tensor_asymmetric_claude);
    m.impl("fakequant_per_tensor_asymmetric_backward_claude", &fakequant_per_tensor_asymmetric_backward_claude);
}
