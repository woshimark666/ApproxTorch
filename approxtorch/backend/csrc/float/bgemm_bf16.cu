...
void bgemm_bf16_tiled_kernel(
  ...
  __shared__ __align__(16) __nv_bfloat16 shared_x[kKTile * kLengthTile];
  __shared__ __align__(16) __nv_bfloat16 shared_w[kKTile * kSharedWStride];
...
#pragma unroll
        for (int item = 0; item < outputs_per_lane; ++item) {
          ...
          const __nv_bfloat16 product =
              float_backend::approx_mul_bf16(lhs, rhs, lut);
...
void launch_bgemm_bf16_tiled(
  ...
  bgemm_bf16_tiled_kernel<outputs_per_lane>
...
torch::Tensor launch_bgemm_bf16(
  ...
  check_lut(lut, torch::kUInt16, 128, X.device(), op_name);
  ...
  if (!optimized || K < kKTile || L < 192 || O < 16) {
