...
) 
{
  ...
  // --- Main Loop ---
    for (uint bkIdx = 0; bkIdx < K; bkIdx += BK) 
    {
        ...
        approxtorch::processFromSmemMasked_uint8<BM, BN, BK, WM, WN, WMITER, WNITER, WSUBM, WSUBN, TM, TN>(
...
torch::Tensor 
approx_gemm_uint8(const torch::Tensor& A, const torch::Tensor& B, const torch::Tensor& lut) 
{
...
) 
{
  ...
  // --- Main Loop ---
    for (uint bkIdx = 0; bkIdx < K; bkIdx += BK) 
    {
        ...
        // 3. process the shared memory
        approxtorch::processFromSmemMasked_int8<BM, BN, BK, WM, WN, WMITER, WNITER, WSUBM, WSUBN, TM, TN>(
            regM, regN, threadResults, As, Bs, 
            warpRow, warpCol, threadRowInWarp, threadColInWarp, 
            validK, lut);  
        __syncthreads();
...
torch::Tensor 
approx_gemm_int8(const torch::Tensor& A, const torch::Tensor& B, const torch::Tensor& lut) 
{
    const at::cuda::OptionalCUDAGuard device_guard(device_of(A));
    int device_id = A.get_device();
    uint M = A.size(0);
...
TORCH_LIBRARY_FRAGMENT(approxtorch, m){
    ...
    m.def("approx_gemm_int8(Tensor A, Tensor B, Tensor lut) -> Tensor");
}
TORCH_LIBRARY_IMPL(approxtorch, CUDA, m){
    m.impl("approx_gemm_uint8", &approx_gemm_uint8);
    m.impl("approx_gemm_int8", &approx_gemm_int8);
}
