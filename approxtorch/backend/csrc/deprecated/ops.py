...
# will not use this, don't know why this is slow 
...
def approx_bgemm_int8(A: Tensor, B: Tensor, lut: Tensor) -> Tensor:
    return torch.ops.approxtorch.approx_bgemm_int8.default(A, B, lut)

def approx_gemm_uint8(A: Tensor, B: Tensor, lut: Tensor) -> Tensor:
    return torch.ops.approxtorch.approx_gemm_uint8.default(A, B, lut)

def approx_gemm_int8(A: Tensor, B: Tensor, lut: Tensor) -> Tensor:
    return torch.ops.approxtorch.approx_gemm_int8.default(A, B, lut)
...
def im2col_int8(feature: Tensor, k_h: int, k_w: int, pad_h: int, pad_w: int, stride_h: int, stride_w: int, dil_h: int, dil_w: int) -> Tensor:
    return torch.ops.approxtorch.im2col_int8.default(feature, k_h, k_w, pad_h, pad_w, stride_h, stride_w, dil_h, dil_w)