...
__global__ void bgemm_custom_grad_dx_int8_naive_kernel(
...
)
{
    ...
    for (int o = 0; o < O; ++o) {
        int w_val = (int)W[o * CKK + c];
        float dy  = dY[(n * O + o) * L + l];
        float lut = lut_dx[(x_val + 128) * 256 + (w_val + 128)];
        sum += dy * lut;
    }
...
__global__ void bgemm_custom_grad_dw_int8_naive_kernel(
    ...
    for (int n = 0; n < N; ++n) {
        for (int l = 0; l < L; ++l) {
            int x_val = (int)X[(n * CKK + c) * L + l];
            float dy  = dY[(n * O + o) * L + l];
            float lut = dw_lut[(x_val + 128) * 256 + (w_val + 128)];
            sum += dy * lut;
        }
    }
...
// Python binding
// ===========================================================================
std::tuple<torch::Tensor, torch::Tensor> bgemm_custom_grad_int8_naive(
    ...
    // Launch grad_X
    {
        int total = N * CKK * L;
        int blocks = (total + threads - 1) / threads;
        bgemm_custom_grad_dx_int8_naive_kernel<<<blocks, threads>>>(
            X.data_ptr<int8_t>(),
            W.data_ptr<int8_t>(),
            dY.data_ptr<float>(),
            dx_lut.data_ptr<float>(),
    ...
    // Launch grad_W
    {
        int total = O * CKK;
        int blocks = (total + threads - 1) / threads;
        bgemm_custom_grad_dw_int8_naive_kernel<<<blocks, threads>>>(
            X.data_ptr<int8_t>(),
            W.data_ptr<int8_t>(),
            dY.data_ptr<float>(),
            dw_lut.data_ptr<float>(),
...
TORCH_LIBRARY_FRAGMENT(approxtorch, m){
    ...
    m.def("bgemm_custom_grad_int8_naive(Tensor X, Tensor W, Tensor dY, Tensor dx_lut, Tensor dw_lut) -> (Tensor, Tensor)");
}

TORCH_LIBRARY_IMPL(approxtorch, CUDA, m){
    m.impl("bgemm_custom_grad_uint8_naive", &bgemm_custom_grad_uint8_naive);
    m.impl("bgemm_custom_grad_int8_naive", &bgemm_custom_grad_int8_naive);
}
