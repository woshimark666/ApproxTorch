...
__global__ void bqsg_backward_x_kernel(
...
__global__ void bqsg_backward_w_kernel(
    const float* __restrict__ grad_output,  // [N, O, L]
    const float* __restrict__ x,            // [N, K, L]
    const float* __restrict__ w,            // [O, K]
...
void check_bqsg_inputs(
    ...
    const torch::Tensor& coeff_deriv
) {
...
std::vector<torch::Tensor> bgemm_bqsg_backward_cuda(
    ...
    bqsg_backward_x_kernel<<<grid_x, block_x, 0, stream>>>(
    ...
    bqsg_backward_w_kernel<<<grid_w, block_w, 0, stream>>>(
        grad_output.data_ptr<float>(),
        x.data_ptr<float>(),
        w.data_ptr<float>(),
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("bqsg_backward", &bgemm_bqsg_backward_cuda,
          "BQSG BGEMM backward, returns [grad_x, grad_w] (CUDA)");
    m.def("set_bqsg_coeff", &set_bqsg_coeff_cuda,
          "Copy BQSG derivative coefficients [16,6] to CUDA constant memory");
}
