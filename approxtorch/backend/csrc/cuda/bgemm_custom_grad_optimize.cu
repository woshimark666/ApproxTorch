...
#define DX_TILE_L_I8   64
#define DX_TILE_CKK_I8 4
#define DX_TILE_O_I8   32
// ============================================================================
// INT8 grad_X kernel — shared memory tiling over O
//
// Same structure as uint8 version, but:
//   - X and W are int8 (range -128..127)
//   - LUT indexing: dx_lut[(x_val + 128) * 256 + (w_val + 128)]
// ============================================================================
__global__ void bgemm_custom_grad_dx_int8_kernel(
    const int8_t*  __restrict__ X,       // (N, CKK, L)
    const int8_t*  __restrict__ W,       // (O, CKK)
    const float*   __restrict__ dY,      // (N, O, L)
    const float*   __restrict__ dx_lut,  // (65536,)
    float*         __restrict__ grad_X,  // (N, CKK, L)
    int N, int CKK, int O, int L)
{
    int n        = blockIdx.z;
    int ckk_base = blockIdx.y * DX_TILE_CKK_I8;
    int l_base   = blockIdx.x * DX_TILE_L_I8;

    int l_local   = threadIdx.x;  // 0..DX_TILE_L_I8-1
    int ckk_local = threadIdx.y;  // 0..DX_TILE_CKK_I8-1
    ...
    // LUT row base: (x_val + 128) * 256
    int lut_row_base = ((int)x_val + 128) << 8;

    // Shared memory
    __shared__ int8_t s_W[DX_TILE_O_I8][DX_TILE_CKK_I8];
    __shared__ float  s_dY[DX_TILE_O_I8][DX_TILE_L_I8];

    int flat_tid = ckk_local * DX_TILE_L_I8 + l_local;
    int block_threads = DX_TILE_L_I8 * DX_TILE_CKK_I8;  // 256

    float acc = 0.0f;

    for (int o_tile = 0; o_tile < O; o_tile += DX_TILE_O_I8) {

        // --- Load W tile: [DX_TILE_O_I8, DX_TILE_CKK_I8] ---
        int w_count = DX_TILE_O_I8 * DX_TILE_CKK_I8;
        for (int i = flat_tid; i < w_count; i += block_threads) {
            int o_loc = i / DX_TILE_CKK_I8;
            int c_loc = i % DX_TILE_CKK_I8;
            int o_idx = o_tile + o_loc;
            int c_idx = ckk_base + c_loc;
            if (o_idx < O && c_idx < CKK) {
                s_W[o_loc][c_loc] = W[(size_t)o_idx * CKK + c_idx];
        ...
        // --- Load dY tile: [DX_TILE_O_I8, DX_TILE_L_I8] ---
        int dy_count = DX_TILE_O_I8 * DX_TILE_L_I8;
        for (int i = flat_tid; i < dy_count; i += block_threads) {
            int o_loc = i / DX_TILE_L_I8;
            int l_loc = i % DX_TILE_L_I8;
            int o_idx = o_tile + o_loc;
            int l_real = l_base + l_loc;
            if (o_idx < O && l_real < L) {
                s_dY[o_loc][l_loc] = dY[(size_t)n * O * L + (size_t)o_idx * L + l_real];
        ...
        // --- Compute: accumulate over this O tile ---
        if (valid) {
            int tile_end = min(DX_TILE_O_I8, O - o_tile);
            #pragma unroll 8
            for (int o_loc = 0; o_loc < tile_end; ++o_loc) {
                float dy_val   = s_dY[o_loc][l_local];
                int8_t w_val   = s_W[o_loc][ckk_local];
                // LUT index: (x_val+128)*256 + (w_val+128)
                int lut_idx    = lut_row_base + (int)w_val + 128;
                float lut_val  = __ldg(&dx_lut[lut_idx]);
                acc += dy_val * lut_val;
            }
        }
...
// ============================================================================
// INT8 grad_W kernel — LUT column preload + large tile + warp shuffle reduction
//
// Same structure as uint8 version, but:
//   - X and W are int8 (range -128..127)
//   - LUT indexing: dw_lut[(x_val + 128) * 256 + (w_val + 128)]
//   - Preload column: s_lut[i] = dw_lut[i * 256 + (w_val + 128)]  for i = 0..255
//     then lookup: s_lut[x_val + 128]
// ============================================================================

#define DW_BLOCK_SIZE_I8 256

__global__ void bgemm_custom_grad_dw_int8_kernel(
    const int8_t*  __restrict__ X,       // (N, CKK, L)
    const int8_t*  __restrict__ W,       // (O, CKK)
    const float*   __restrict__ dY,      // (N, O, L)
    const float*   __restrict__ dw_lut,  // (65536,)
    ...
    int N, int CKK, int O, int L)
{
    ...
    // Load w_val — constant for this entire block
    int8_t w_val = W[(size_t)o_idx * CKK + ckk_idx];
    int w_col = (int)w_val + 128;  // shift to 0..255

    // Preload LUT column into shared memory:
    // s_lut[i] = dw_lut[i * 256 + (w_val + 128)]  for i = 0..255
    // Index i corresponds to (x_val + 128), so lookup is s_lut[x_val + 128]
    __shared__ float s_lut[256];
    if (tid < 256) {
        s_lut[tid] = dw_lut[tid * 256 + w_col];
    }
    __syncthreads();
    ...
    for (int nl = tid; nl < total_nl; nl += DW_BLOCK_SIZE_I8) {
        int n = nl / L;
        int l = nl % L;

        int8_t x_val = *reinterpret_cast<const int8_t*>(
            &X[(size_t)n * CKK * L + (size_t)ckk_idx * L + l]);
        float dy_val = __ldg(&dY[(size_t)n * O * L + (size_t)o_idx * L + l]);

        // LUT lookup from shared memory: s_lut[x_val + 128]
        acc += dy_val * s_lut[(int)x_val + 128];
    }
    ...
    // Final reduction by first warp
    if (warp_id == 0) {
        acc = (lane_id < (DW_BLOCK_SIZE_I8 / 32)) ? s_reduce[lane_id] : 0.0f;
        #pragma unroll
        for (int offset = 16; offset > 0; offset >>= 1) {
            acc += __shfl_down_sync(0xFFFFFFFF, acc, offset);
        }
...
torch::Tensor bgemm_custom_grad_int8_dx(
    torch::Tensor X,       // (N, CKK, L) int8
    torch::Tensor W,       // (O, CKK) int8
    torch::Tensor dY,      // (N, O, L) float32
    torch::Tensor dx_lut   // (65536,) float32
) {
    TORCH_CHECK(X.is_cuda() && W.is_cuda() && dY.is_cuda());
    TORCH_CHECK(X.dtype() == torch::kInt8);
    TORCH_CHECK(W.dtype() == torch::kInt8);
    TORCH_CHECK(dY.dtype() == torch::kFloat32);
    TORCH_CHECK(dx_lut.dtype() == torch::kFloat32);
    ...
    dim3 block(DX_TILE_L_I8, DX_TILE_CKK_I8);  // (64, 4) = 256 threads
    dim3 grid(
        (L   + DX_TILE_L_I8   - 1) / DX_TILE_L_I8,
        (CKK + DX_TILE_CKK_I8 - 1) / DX_TILE_CKK_I8,
        N
    );

    bgemm_custom_grad_dx_int8_kernel<<<grid, block, 0, at::cuda::getCurrentCUDAStream()>>>(
        X.data_ptr<int8_t>(),
        W.data_ptr<int8_t>(),
        dY.data_ptr<float>(),
        dx_lut.data_ptr<float>(),
...
torch::Tensor bgemm_custom_grad_int8_dw(
    torch::Tensor X,       // (N, CKK, L) int8
    torch::Tensor W,       // (O, CKK) int8
    torch::Tensor dY,      // (N, O, L) float32
    torch::Tensor dw_lut   // (65536,) float32
    ...
    dim3 block(DW_BLOCK_SIZE_I8);  // 256
    dim3 grid(O, CKK);

    bgemm_custom_grad_dw_int8_kernel<<<grid, block, 0, at::cuda::getCurrentCUDAStream()>>>(
        X.data_ptr<int8_t>(),
        W.data_ptr<int8_t>(),
        dY.data_ptr<float>(),
        dw_lut.data_ptr<float>(),
...
TORCH_LIBRARY_FRAGMENT(approxtorch, m) {
    ...
    m.def("bgemm_custom_grad_int8_dx(Tensor X, Tensor W, Tensor dY, Tensor dx_lut) -> Tensor");
    m.def("bgemm_custom_grad_int8_dw(Tensor X, Tensor W, Tensor dY, Tensor dw_lut) -> Tensor");
}
...
TORCH_LIBRARY_IMPL(approxtorch, CUDA, m) {
    ...
    m.impl("bgemm_custom_grad_int8_dx", &bgemm_custom_grad_int8_dx);
    m.impl("bgemm_custom_grad_int8_dw", &bgemm_custom_grad_int8_dw);
}
