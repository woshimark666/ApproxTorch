...
class conv2d_T_class(Function):
    @staticmethod
    def forward(feature, weight, lut, T_feature, T_weight, bias=None, stride:int=1, padding:int=0, dilation:int=1):
        (B, C, H, W) = feature.shape
        (O, C, _, K) = weight.shape
        OH = math.floor((H + 2*padding - dilation*(K-1) - 1)/stride + 1)
        OW = math.floor((W + 2*padding - dilation*(K-1) - 1)/stride + 1)
...
class conv2d_est_T(Function):
    @staticmethod
    def forward(ctx, feature, weight, lut, gradient_lut, T_feature, T_weight, bias=None, stride:int=1, padding:int=0, dilation:int=1):
        (B, C, H, W) = feature.shape
        (O, C, Kh, Kw) = weight.shape
        ctx.stride = stride
        ctx.padding = padding
    ...
    def backward(ctx, upstream_grad):
        ...
        return grad_feature, grad_weight, None, None, None, None, grad_bias, None, None, None  
...
def conv2d_int8_T(feature, weight, lut, T_feature, T_weight, bias=None, stride:int = 1, padding:int = 0, dilation:int = 1):
    return conv2d_T_class.apply(feature, weight, lut, T_feature, T_weight, bias, stride, padding, dilation)

def conv2d_int8_est_T(feature, weight, lut, gradient_lut, T_feature, T_weight, bias=None, stride:int = 1, padding:int = 0, dilation:int = 1):
    return conv2d_est_T.apply(feature, weight, lut, gradient_lut, T_feature, T_weight, bias, stride, padding, dilation)