...
__global__ void apgemm_2d_thread_tiling_matrix_transposed_kernel_small(int M , int N, int K, 
...
#pragma unroll
                    ...
                    thread_tile_row_idx < TM; ++thread_tile_row_idx)
            {
                        ...
                        thread_tile_col_idx < TN; ++thread_tile_col_idx)
                {
                    ...
                    uint const lut_idx = 256 * A_vals[thread_tile_row_idx] + B_vals[thread_tile_col_idx] + 32896;
...
template <size_t BM, size_t BN, size_t BK, size_t TM, size_t TN>
__global__ void apgemm_2d_thread_tiling_matrix_transposed_kernel_big(int M , int N, int K, 
...
)
{
    ...
    if( MK < INT32_MAX)
        apgemm_2d_thread_tiling_matrix_transposed_kernel_small<BM, BN, BK, TM, TN><<<grid_dim, block_dim,0, at::cuda::getCurrentCUDAStream()>>>(
            M, N, K,
    ...
    else apgemm_2d_thread_tiling_matrix_transposed_kernel_big<BM, BN, BK, TM, TN><<<grid_dim, block_dim,0, at::cuda::getCurrentCUDAStream()>>>(
            M, N, K,
