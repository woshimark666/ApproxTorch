...
__global__ void apgemm_2d_thread_tiling_matrix_transposed_uint8_kernel_small(int M , int N, int K, 
                    ...
                    torch::PackedTensorAccessor32<int, 2>  C)
{
    ...
    {
        load_to_shared_memory_transposed_uint8<BM, BN, BK, NUM_THREADS>(
...
template <size_t BM, size_t BN, size_t BK, size_t TM, size_t TN>
__global__ void apgemm_2d_thread_tiling_matrix_transposed_uint8_kernel_big(int M , int N, int K, 
...
)
{
    ...
    if( MK < INT32_MAX)
        apgemm_2d_thread_tiling_matrix_transposed_uint8_kernel_small<BM, BN, BK, TM, TN>
        <<<grid_dim, block_dim,0, at::cuda::getCurrentCUDAStream()>>>(
            M, N, K,
            A.packed_accessor32<uint8_t, 2>(),
            B.packed_accessor32<uint8_t, 2>(),
            lut.packed_accessor32<int32_t, 1>(),
            C.packed_accessor32<int, 2>()
        );
    else apgemm_2d_thread_tiling_matrix_transposed_uint8_kernel_big<BM, BN, BK, TM, TN>
        <<<grid_dim, block_dim,0, at::cuda::getCurrentCUDAStream()>>>(
            M, N, K,
            A.packed_accessor64<uint8_t, 2>(),
            B.packed_accessor32<uint8_t, 2>(),
            lut.packed_accessor32<int32_t, 1>(),
