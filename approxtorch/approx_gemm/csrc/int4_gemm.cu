...
__global__ void apgemm_2d_thread_tiling_matrix_transposed_int4_kernel_small(int M , int N, int K, 
                    const torch::PackedTensorAccessor32<int8_t, 2>  A,
                    const torch::PackedTensorAccessor32<int8_t, 2>  B,
                    const torch::PackedTensorAccessor32<int8_t, 1>  lut,
                    torch::PackedTensorAccessor32<int, 2>  C)
{
    ...
    __shared__ int8_t lut_s[256];
    // fist is to load lut to shared memory
...
#pragma unroll
                    ...
                    thread_tile_row_idx < TM; ++thread_tile_row_idx)
            {
                        ...
                        thread_tile_col_idx < TN; ++thread_tile_col_idx)
                {
                    ...
                    C_thread_results[thread_tile_row_idx][thread_tile_col_idx] +=  lut_s[lut_idx];   
                }
            }
        }
        __syncthreads();
...
template <size_t BM, size_t BN, size_t BK, size_t TM, size_t TN>
__global__ void apgemm_2d_thread_tiling_matrix_transposed_int4_kernel_big(int M , int N, int K, 
...
)
{
    ...
    if( MK < INT32_MAX)
        apgemm_2d_thread_tiling_matrix_transposed_int4_kernel_small<BM, BN, BK, TM, TN><<<grid_dim, block_dim,0, at::cuda::getCurrentCUDAStream()>>>(
            M, N, K,
            A.packed_accessor32<int8_t, 2>(),
            B.packed_accessor32<int8_t, 2>(),
            lut.packed_accessor32<int8_t, 1>(),
            C.packed_accessor32<int, 2>()
        );
    else apgemm_2d_thread_tiling_matrix_transposed_int4_kernel_big<BM, BN, BK, TM, TN><<<grid_dim, block_dim,0, at::cuda::getCurrentCUDAStream()>>>(
            M, N, K,
