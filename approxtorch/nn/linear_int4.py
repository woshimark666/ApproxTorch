...
class _linear_int4_STE(Function):
    ...
    def forward(feature: torch.Tensor, 
        ...
        match qmethod:
            case 'dynamic':
                feature, scale_feature = Q.quantize_dynamic_int4_per_tensor(feature, (0,1))
                weight, scale_weight = Q.quantize_dynamic_int4_per_tensor(weight, (0,1))
        ...
        output = ap.ops.gemm_int4(feature, weight, lut).to(torch.float)  # (B, out_features)
...
def linear_int4_STE(feature, 
                    weight, 
                    lut, 
                    qmethod: str = 'dynamic', 
                    qparams: Tuple[torch.Tensor, torch.Tensor] | None = None, 
                    bias=None):
    return _linear_int4_STE.apply(feature, weight, lut, qmethod, qparams, bias)