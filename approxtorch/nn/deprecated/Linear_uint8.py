import torch
from . import linear_uint8

class Linear_uint8_STE(torch.nn.Module):
    def __init__(self, 
                 in_features: int,
                 out_features: int,
                 lut: torch.Tensor,
    ...
    def __repr__(self):
        return f"Linear_uint8_STE(in_features={self.in_features}, " \
               f"out_features={self.out_features}, " \
               f"qmethod={self.qmethod}, bias={self.has_bias})"
    
    def forward(self, x):
        return linear_uint8.linear_uint8_STE(x,
                    self.weight,
                    self.lut,
                    self.qmethod,
                    self.qparams,
