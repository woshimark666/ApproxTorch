"""LUT-based approximate FP16 Linear layer."""
...
__all__ = ["Linear_float16", "Linear_fp16", "linear_float16", "linear_fp16"]
...
def _validate_linear_tensors(
    ...
    if not all(tensor.is_cuda for tensor in tensors):
        raise ValueError("input, weight, lut, and bias must be CUDA tensors")
    if input.dtype != torch.float16 or weight.dtype != torch.float16:
        raise TypeError(
            "input and weight must have dtype torch.float16, got "
            f"{input.dtype} and {weight.dtype}"
        )
    if bias is not None and bias.dtype != torch.float16:
        raise TypeError(f"bias must have dtype torch.float16, got {bias.dtype}")
...
class _LinearFloat16STE(Function):
    @staticmethod
    def forward(
        ctx,
        ...
        if optimized:
            return ops.gemm_fp16(input_2d, transposed_weight, lut)
        return ops.gemm_fp16_naive(input_2d, transposed_weight, lut)
...
def linear_float16(
    ...
    """Apply an approximate FP16 linear transform with an exact-product STE."""
    ...
    output = _LinearFloat16STE.apply(input_2d, weight.contiguous(), lut, optimized)
...
class Linear_float16(nn.Module):
    """Approximate FP16 Linear layer with an exact-product STE backward."""
    ...
    def __init__(
        ...
        if dtype is not None and dtype != torch.float16:
            raise TypeError(
                f"fp16 linear requires dtype torch.float16, got {dtype}"
            )
        if not isinstance(lut, torch.Tensor):
            raise TypeError(f"lut must be a torch.Tensor, got {type(lut).__name__}")
        ...
        if supplied_bias is not None:
            with torch.no_grad():
                self.bias.copy_(
                    supplied_bias.to(device=target_device, dtype=torch.float16)
                )
